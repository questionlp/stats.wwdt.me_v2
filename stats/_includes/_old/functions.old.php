...
function parsePanelists($showDate) {
	...
	if (is_null($panelistScores) || (!(array_key_exists($showDate, $panelistScores)))) {
		$returnValue = '<span class="nadata">(N/A)</span>';
		return $returnValue;
	}
	...
	if (is_null($scores)) {
		$returnValue = '<em>(Multiple Panelists)</em>';
		return $returnValue;
	}
	...
	foreach (array_keys($scores) as $panelist) {
		...
		if ($panelist == 'Luke Burbank') {
			$cPanelist = '<span title="Luuuuuuke">' . htmlentities($panelist) . '</span>';
		} else {
			$cPanelist = htmlentities($panelist);
		}

		if ($score != null) {
			switch ($rank) {
				case '1':
					$returnValue .= '<span class="rank1">' . $cPanelist . ' (' . $score . ')</span>' . "\n";
					break;
				case '1t':
					$returnValue .= '<span class="rank1t">' . $cPanelist . ' (' . $score . ')</span>' . "\n";
					break;
				case '2':
					$returnValue .= '<span class="rank2">' . $cPanelist . ' (' . $score . ')</span>' . "\n";
					break;
				case '2t':
					$returnValue .= '<span class="rank2t">' . $cPanelist . ' (' . $score . ')</span>' . "\n";
					break;
				case '3':
					$returnValue .= '<span class="rank3">' . $cPanelist . ' (' . $score . ')</span>' . "\n";
					break;
				default:
...
function parseGuests($showDate) {
	...
	foreach (array_keys($scores) as $guest) {
		if ($guest == '[None]') {
			$returnValue .= '<span class="noGuest">No Guest</span>';
			break;
		}
		...
		if ($score != null) {
			...
			if (($guest == 'Maj. Robert Bateman') && ($score == 1)) {
				...
				if ($showDate == '2005-06-25') {
					 ...
					 */
					$returnValue .= '<span class="guestWon">' . htmlentities($guest) . ' (' . $score . ')</span>' . '<a href="#fn1" id="ofn1"><sup class="fn">1</sup></a>' . "\n";
				} else {
					$returnValue .= '<span class="guestWon">' . htmlentities($guest) . ' (' . $score . ')</span>' . '<a href="#fn1"><sup class="fn">1</sup></a>' . "\n";
				}
				...
				if ($showDate == '2007-07-21') {
					 ...
					 */
					$returnValue .= '<span class="guestWon">' . htmlentities($guest) . ' (' . $score . ')</span>' . '<a href="#fn2" id="ofn2"><sup class="fn">2</sup></a>' . "\n";
				} else {
					$returnValue .= '<span class="guestWon">' . htmlentities($guest) . ' (' . $score . ')</span>' . '<a href="#fn2"><sup class="fn">2</sup></a>' . "\n";
				}
			} else if ($score >= 2) {
				$returnValue .= '<span class="guestWon">' . htmlentities($guest) . ' (' . $score . ')</span>' . "\n";
			} else {
				$returnValue .= htmlentities($guest) . ' (' . $score . ')' . "\n";
			}
		} else {
			$returnValue .= htmlentities($guest) . "\n";
		}
	}
...
function processShowURL($showDate) {
	...
	$currentURLTemplate = 'http://www.npr.org/templates/rundowns/rundown.php?prgId=35&amp;prgDate=';
	$legacyURLTemplate = 'http://www.npr.org/programs/waitwait/archrndwn/';
	
	if ($dtShowDate >= $wwNewFormatDate) {
		$urlShowDate = $currentURLTemplate . $dtShowDate->format('m-d-Y');
		return $urlShowDate;
	} else {
		$showDateFmt = $dtShowDate->format('ymd');
		$showYearFmt = $dtShowDate->format('Y');
		$monthName = strtolower($dtShowDate->format('M'));
		$urlShowDate = $legacyURLTemplate . $showYearFmt . '/' . $monthName . '/' . $showDateFmt . '.waitwait.html';
		return $urlShowDate;
	}
}
...
function generateShowInfoBlock($showDate, $showRating, $showFullDescription, $showHost, $showSK) {
	...
	$infoBlock .= "<span class=\"showDate\" id=\"s$showDateID\">";
	...
	if (!is_null($showRating)) {
		$infoBlock .= "\n<br /><br /><span class=\"showRating\">Rating:</span> $showRating\n";
	}
	
	$infoBlock .= "</td>\n";
	
	if (!empty($showFullDescription)) {
		$infoBlock .= '<td colspan="4" class="showDesc">' . $showFullDescription . "</td>\n";
		$infoBlock .= "</tr>\n";
		$infoBlock .= "<tr>\n";
	}
